 ...
 SELECT * FROM ADVENTUREWORKS.SALESLT.Product


 use master;
 ALTER LOGIN Naster WITH PASSWORD = 'ZAQxswcde123±';
...
SCHEMA_NAME NOT LIKE 'db\_%' ESCAPE '\';
 ...
 0 AS IsUnsigned,   
 ISNULL((         SELECT TOP 1 1         FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE KCU         
 ...
 WHERE             KCU.TABLE_CATALOG = COL.TABLE_CATALOG AND             KCU.TABLE_SCHEMA = COL.TABLE_SCHEMA AND
              KCU.TABLE_NAME = COL.TABLE_NAME AND             KCU.COLUMN_NAME = COL.COLUMN_NAME AND            
			   TC.CONSTRAINT_TYPE = 'PRIMARY KEY'     ), 0) AS 'IsPrimaryKey',    
COLUMNPROPERTY(object_id(COL.TABLE_NAME), COL.COLUMN_NAME, 'IsIdentity') AS 'IsAutoIncrement' 
FROM INFORMATION_SCHEMA.COLUMNS COL INNER JOIN INFORMATION_SCHEMA.TABLES TBL ON TBL.TABLE_CATALOG = COL.TABLE_CATALOG AND
       TBL.TABLE_SCHEMA = COL.TABLE_SCHEMA AND               TBL.TABLE_NAME = COL.TABLE_NAME AND   
	     TBL.TABLE_TYPE = 'BASE TABLE' 
		 --WHERE COL.TABLE_SCHEMA = @DATABASE
		 --where column_name = 'REFERENCED_TABLE_SCHEMA'
...
EXEC sp_addsrvrolemember 'naster', 'sysadmin';
GRANT CONTROL SERVER TO naster;


SELECT * FROM master.sys.database_permissions

GRANT SELECT TO NASTER